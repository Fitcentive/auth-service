...
object JwtConfig {
  def apply(config: Config): JwtConfig =
    apply("issuer", "expiration", "encryption", "secret", config)

  def apply(_issuer: String, _expiration: String, _encryption: String, _secret: String, config: Config): JwtConfig = {
  ...
  private def build[T](issuer: String, expiration: Long, _encryption: String, secret: String): JwtConfig = {
    val encryption =
      _encryption match {
        case "HS256" => JwtAlgorithm.HS256
        case "HS384" => JwtAlgorithm.HS384
        case "HS512" => JwtAlgorithm.HS512
        case _       => JwtAlgorithm.HS256
      }
    JwtConfig(issuer, expiration, encryption, secret)
  }
}
